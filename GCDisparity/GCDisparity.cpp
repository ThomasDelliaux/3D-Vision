...
//This function is the function rho given in the course. return 1 if x<0 and sqrt(1-x) otherwise.
double rho(double x){
    if(x>=0 && x<=1){
        return sqrt(1-x);
    }
...
//Associate a point (x,y,d) to a unique node number. Since this map depends of nx and ny we have to specify them.
int getNbNode(int x,int y,int d,int nx,int ny){
    return x+nx*y+nx*ny*d;
}
...
void build_graph(Graph<int,int,int>& G,
    ...
    //Compute K in order to had it at wcc*rho(ncc) to avoid multiple cut.
    int K = 1+nd*4*lambda;
    G.add_node(nx*ny*nd);
    ...
    for(int x=0; x<nx; ++x){
        for(int y=0; y<ny; ++y){
            for(int d=0; d<nd; ++d){
               //get the coordiantes in the image 1 of point (x,y) in order to compute the zncc.
               int u = win+zoom*x;
               int v = win+zoom*y;
               //Connect the source to the "first layer".
                if(d==0){

                    float ncc = zncc(I1,I1M,I2,I2M,u,v,u+dmin,v);
                    G.add_tweights(getNbNode(x,y,d,nx,ny),K+wcc*rho(ncc),0);
                }
                //Connect the "last layer" to the sink.
                if(d==nd-1){
                    float ncc = zncc(I1,I1M,I2,I2M,u,v,u+dmax,v);
                    G.add_tweights(getNbNode(x,y,d,nx,ny),0,K+wcc*rho(ncc));
                }
                //add edges between the node associate to (x,y,d-1) and the node associate to (x,y,d). The weight is K+wcc*rho(ncc).
                //The ncc is evalute with respect to d.
                if(d>=1){
                        float ncc = zncc(I1,I1M,I2,I2M,u,v,u+d+dmin,v);
                        G.add_edge(getNbNode(x,y,d-1,nx,ny),getNbNode(x,y,d,nx,ny),K+wcc*rho(ncc),0);


                }
                //add egdes between the node associate to (x,y,d) and the node associate to (x+1,y,d). The weight is lambda in the two directions.
                if(x+1<nx){
                    G.add_edge(getNbNode(x,y,d,nx,ny),getNbNode(x+1,y,d,nx,ny),lambda,lambda);
                }
                //add egdes between the node associate to (x,y,d) and the node associate to (x,y+1,d). The weight is lambda in the two directions.
                if(y+1<ny){
                    G.add_edge(getNbNode(x,y,d,nx,ny),getNbNode(x,y+1,d,nx,ny),lambda,lambda);
                }
...
doubleImage decode_graph(Graph<int,int,int>& G, int nx, int ny, int nd) {
    ...
    //Fill D with dmax because we will check for each node wether is in sink side. So for a point (x,y) if for all d the node
    //associate to (x,y,d) is not in the sink side, that means that the disparity for (x,y) is dmax.
    D.fill(dmax);
    for(int y=0; y<ny; y++){
        for(int x=0; x<nx; x++) {
            //For (x,y) we check for all d if the node associate to (x,y,d) is in the sink side. At the first d for which is true, we put d+dmin
            //for the disparity of (x,y).
            for(int d=0; d<nd; ++d){
                if(G.what_segment(getNbNode(x,y,d,nx,ny))==Graph<int,int,int>::SINK){
                    D(x,y) = d+dmin;
                    break;
                }
